...
class ChatbotController extends Controller
{
    ...
    {
        return $this->faqs[$message] ?? null;
    }

    protected function getNextFAQs($currentQuestion = null)
    {
        $faqKeys = array_keys($this->faqs);
        $index = ($currentQuestion && in_array($currentQuestion, $faqKeys)) 
                 ? array_search($currentQuestion, $faqKeys) + 1 : 0;

        $suggestions = array_slice($faqKeys, $index, 3);
        if (count($suggestions) < 3) {
            $suggestions = array_merge($suggestions, array_slice($faqKeys, 0, 3 - count($suggestions)));
        }
